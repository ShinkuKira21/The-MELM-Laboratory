...
# Allow running from repo root or MELM_Generator/.
...
def main():
    ...
    parser.add_argument("--config", default="../config.generator.json")
    ...
    if not os.path.isfile(train_path) or not os.path.isfile(valid_path):
        raise SystemExit(
            "Missing Architecture-3 training/validation data.\n"
            f"Expected:\n"
            f"  train: {train_path}\n"
            f"  valid: {valid_path}\n\n"
            "Fix:\n"
            "  - Put `domain_train.jsonl` and `domain_valid.jsonl` under `architecture-3/data/`, OR\n"
            "  - Update `config.generator.json` -> data.train/data.valid to point to your Arch-3 data files.\n"
        )
