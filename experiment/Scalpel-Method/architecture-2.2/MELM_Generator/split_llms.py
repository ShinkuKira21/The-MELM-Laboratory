...
def main():
    ...
    if not os.path.isfile(train_path):
        raise SystemExit(
            "Missing Architecture-3 training data.\n"
            f"Expected:\n"
            f"  train: {train_path}\n\n"
            "Fix:\n"
            "  - Put `domain_train.jsonl` under `architecture-3/data/`, OR\n"
            "  - Update `config.generator.json` -> data.train to point to your Arch-3 train file.\n"
        )
